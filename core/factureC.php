...
class factureC {
	...
	function modifierFacture($facture,$id){
		$sql="UPDATE facture SET  id_cmd=:id_cmd , nom_prenom=:nom_prenom, dat=:dat, adresse=:adresse, region=:region, ville=:ville, mode_livraison=:mode_livraison, mode_paiement=:mode_paiement , id_prod=:id_prod , nom_prod=:nom_prod , quantite=:quantite , taille=:taille , prix=:prix , total=:total WHERE id=:id";
...
try{	//$id=$_GET['id'];
        ...
        $datas = array( ':id_cmd'=>$id_cmd , ':nom_prenom'=>$nom_prenom, ':dat'=>$dat,':adresse'=>$adresse,':region'=>$region,':ville'=>$ville,':mode_livraison'=>$mode_livraison, ':mode_paiement'=>$mode_paiement ,':nom_prod'=>$nom_prod , ':quantite'=>$quantite , ':taille'=>$taille , ':prix'=>$prix ,':total'=>$total);
